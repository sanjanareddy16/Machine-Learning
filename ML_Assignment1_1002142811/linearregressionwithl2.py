...
class LinearRegressionwithL2:
    def __init__(self, batch_size=32, regularization=0.1, max_epochs=100, patience=3):
    ...
    def fit(self, X, y, batch_size=32, regularization=0.1, max_epochs=100, patience=3):
        """Fit a linear model.
        ...
        for epoch in range(max_epochs):
            for i in range(0, len(X_train), batch_size):
                ...
                self.weights = (1 - lr * regularization) * self.weights - lr * grad_weights
                self.bias -= lr*grad_bias
                
            # Calculate the loss on the validation set
            val_predictions = np.dot(X_val, self.weights) + self.bias
            val_loss = np.mean((val_predictions - y_val) ** 2) + regularization * np.sum(self.weights ** 2)  # L2 regularization term
